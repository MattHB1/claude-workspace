...
.workspace/
  ...
  tracker-states       # PROJECT-LEVEL tracker transition-point → state-name mapping — shared by all initiatives (see "Tracker write-back" below)
...
Only **four** artefacts live at the `.workspace/` root: the registry `initiatives.md`, the project-level `file-structure.md`, the project-level `namespaces` declaration, and the project-level `tracker-states` mapping. `file-structure.md` is **one file tree per project** (never per-initiative — no `.workspace/<slug>/file-structure.md`); `namespaces` is the project's cross-project membership, **shared by all of the project's initiatives** (never duplicated per initiative); `tracker-states` is the project's transition-point → state-name mapping, likewise **shared by all of the project's initiatives** (never duplicated per initiative — see "Tracker write-back" below). Everything else is initiative-scoped under `<slug>/`.
...
**One status store.** The registry keeps its **single status cell** — the third field above. No second status store, no PR field, and no ticket-state field is added to `initiatives.md` or to its row template. The project-level `.workspace/tracker-states` file holds a **transition-point → state-name mapping only**; it records no per-ticket state, so it is not a second status store.
...
- **Create a new initiative** — triggers: "new initiative <name>", "start a new initiative", "spin up an initiative for…". **Ticket input (one ticket = one initiative).** This verb takes a **ticket identifier** as its input. Use that identifier **as the `<slug>`** — a numeric slug is valid. Get the ticket text one of two ways, in this order: (a) when a tracker tool is available, read the ticket with it; (b) otherwise, ask the user to paste the ticket text. Either way, save the ticket text as a file under the **active initiative's `research/`** (`.workspace/<slug>/research/`), the same way you save a `research-harvester` brief. **Resolve the branch name here too** — see "Git actions" below for the one rule — and create the branch as part of this verb. Action: using that ticket identifier as the `<slug>`, create the `.workspace/<slug>/` subtree (empty `research/`/`verification/`, and the `memory/` subtree on first memory use), add a registry entry (slug + one-line description + status) to `.workspace/initiatives.md`, and **mark it ACTIVE** (demoting whichever was previously active, so exactly one stays active). Then, before offering the reminder, compute the session's age deterministically: read `~/.claude/sessions/"$CLAUDE_CODE_SESSION_ID".json` (the file `record-session-start.py` writes on `SessionStart`, keyed by the full session id) and take its `start_time` field; age = now − `start_time`. If the file is absent or unreadable, treat age as **unknown** and **do not fire** — this mirrors the script's own fail-silent `except: pass` posture, never blocking or erroring the create. Emit the reminder **only when** the computed age is **≥ the threshold**; below the threshold, or when age is unknown, stay **silent** and let the verb complete exactly as it would without this step. The threshold is a single named, documented value: **default 2 hours**, tunable/configurable (mirroring the memory layer's "default 500 KB, configurable" precedent) — adjust it in one place without re-architecting the gate. When the gate fires, say: "Now that <slug> is active, if this session has been running a while a fresh start will cut cost with no loss — all state is durable on disk and re-bootstrapped on re-entry. You can type `/clear` (fresh start) or `/compact` (keeps a summary), then re-invoke `/workspace` to land straight back in <slug>. (Advisory only — I cannot clear context myself.)" This offers the same choice, the same honesty note ("I cannot clear context myself" — only the user can clear context), and the same "re-invoke re-bootstraps → work resumes" assurance as the exit verb's step 3 ("Exiting the workspace"), so the clearing vocabulary stays identical across exit and create/switch. **Reuse / determinism note.** The age signal reused here is `record-session-start.py`'s existing output at `~/.claude/sessions/<session_id>.json` — no new file, hook, script, or env var is introduced; the read is a path lookup + timestamp subtraction only, with no inference or similarity step. `events.jsonl` is **not** used as the age source: its `session` field is an 8-char prefix, not a collision-safe join key against the full `CLAUDE_CODE_SESSION_ID`, and its earliest timestamp is the first tool event, not true session start — both disqualify it as a deterministic age signal. **Index-bloat guard check (prompt-gated, advisory-only — see "Consolidation policy › Index-bloat guard" above).** **Epic/granularity guard check (advisory-only, never-gating — see "Consolidation policy › Epic/granularity guard" above).** Both are wired here too, for symmetry with Switch — but on a **freshly created** initiative the index is empty and the journal has zero entries, so neither check's threshold comparison can ever trip: there is nothing to measure yet. This adds no ceremony that could fire on a brand-new initiative; it exists purely so Create and Switch reference the same two checks consistently.
- **Switch the active initiative** — triggers: "switch to <name>", "make <name> active", "work on <name>". Action: change the ACTIVE marker in `.workspace/initiatives.md` to the named initiative's slug (exactly one ACTIVE at all times); no files move. Then, before offering the reminder, compute the session's age deterministically: read `~/.claude/sessions/"$CLAUDE_CODE_SESSION_ID".json` (the file `record-session-start.py` writes on `SessionStart`, keyed by the full session id) and take its `start_time` field; age = now − `start_time`. If the file is absent or unreadable, treat age as **unknown** and **do not fire** — this mirrors the script's own fail-silent `except: pass` posture, never blocking or erroring the switch. Emit the reminder **only when** the computed age is **≥ the threshold**; below the threshold, or when age is unknown, stay **silent** and let the verb complete exactly as it would without this step. The threshold is a single named, documented value: **default 2 hours**, tunable/configurable (mirroring the memory layer's "default 500 KB, configurable" precedent) — adjust it in one place without re-architecting the gate. When the gate fires, say: "Now that <slug> is active, if this session has been running a while a fresh start will cut cost with no loss — all state is durable on disk and re-bootstrapped on re-entry. You can type `/clear` (fresh start) or `/compact` (keeps a summary), then re-invoke `/workspace` to land straight back in <slug>. (Advisory only — I cannot clear context myself.)" This offers the same choice, the same honesty note ("I cannot clear context myself" — only the user can clear context), and the same "re-invoke re-bootstraps → work resumes" assurance as the exit verb's step 3 ("Exiting the workspace"), so the clearing vocabulary stays identical across exit and create/switch. **Reuse / determinism note.** The age signal reused here is `record-session-start.py`'s existing output at `~/.claude/sessions/<session_id>.json` — no new file, hook, script, or env var is introduced; the read is a path lookup + timestamp subtraction only, with no inference or similarity step. `events.jsonl` is **not** used as the age source: its `session` field is an 8-char prefix, not a collision-safe join key against the full `CLAUDE_CODE_SESSION_ID`, and its earliest timestamp is the first tool event, not true session start — both disqualify it as a deterministic age signal. **Index-bloat guard check (prompt-gated, advisory-only — see "Consolidation policy › Index-bloat guard" above).** After the marker flip, reading only the newly-active initiative's own memory files, derive the byte size of `.workspace/<active-slug>/memory/index.md` (e.g. `wc -c`) and compare it to the same index threshold used at Bootstrap/Teardown (see "Consolidation policy › Index-bloat guard" above). If size ≥ threshold, surface the identical one-line prompt: `index is N KB ≥ threshold T — re-derive it lean (pointers + short summaries only)? y/n`. As at Bootstrap/Teardown, this only proceeds on "y" — never automatic; it is advisory only and never blocks or delays the switch. **Epic/granularity guard check (advisory-only, never-gating — see "Consolidation policy › Epic/granularity guard" above).** In the same step, derive the newly-active initiative's journal entry count via `grep -c '^timestamp:'` over `.workspace/<active-slug>/memory/journal.md` and compare it to the same Epic threshold used at Teardown (see "Consolidation policy › Epic/granularity guard" above). If the count ≥ threshold, surface the identical one-line notice: `this initiative has grown to N — it may be Epic-sized; consider splitting into tighter initiatives`. As at Teardown, this notice never gates on a "y" and never blocks or delays the switch. **Scope note — size checks only, not a Bootstrap.** These two checks reuse only the existing byte-size and entry-count measurements and thresholds defined above; they introduce nothing new. Switching does **not** additionally read the journal tail or ingest the newly-active initiative's canonical artefacts (proposal/tasks) — that remains the required Bootstrap read (see "Bootstrap" above), not part of this verb. This keeps Switch a lightweight registry-marker action, not a Bootstrap.
...
### Git actions — yours, like the registry write and the memory writes

**Branch, commit, push, and open-PR are orchestrator actions.** They sit beside the registry write named above and the memory writes named below (see "Memory layer › Ownership"): **you** perform them. They dispatch no subagent and add no agent — the set stays at **eight**. No agent performs a git action, and git is never an agent's write-target.

**Branch rule (one rule, two sources, no inference).** Use the branch name **the tracker supplies**. When the tracker supplies none, use the **initiative slug**. There is no third source: never build a branch name from the ticket title text, from the description, or from any other guess.

**Prompting split.** **Commit runs with no prompt.** **Push** and **open-PR** each fire a **one-line `y/n` prompt** first, in the same style as the registry-consolidation prompt — for example `push <branch> to origin? y/n` and `open a PR for <branch>? y/n`. Each proceeds only on "y". Push and PR leave the machine and are visible to the user's team, so they are always asked for.

**Where these attach.** They ride the existing flow points and add **no new step and no extra dispatch**: at **Create** you resolve the branch name and create the branch; at the **first build** the work is under way on that branch; **after a task passes verification** you commit, then ask before push, then ask before opening the PR.

### Tracker write-back — prompt-gated, exactly two moments

**Two transition points, and no more.** Exactly two moments fire a tracker write-back prompt: **work started**, and **PR opened** (the handoff). Each fires a **one-line `y/n` prompt**, in the same style as the registry-consolidation prompt — for example `move <slug> to <mapped state>? y/n`. Each writes to the tracker only on "y". There is no third transition point, and no tracker write ever happens without a "y".

**No close.** The plugin proposes no close or Done transition, because closing follows review and merge and is another person's decision.

**State names come from the tracker, never from here.** The plugin hardcodes no state name. The state list is **read from the tracker at mapping time**.

**The mapping file `.workspace/tracker-states` (defined here, once).** It is a project-level plain-text file at the `.workspace/` root, shared by every initiative in the project. **Shape:** one line per transition point — the transition-point key, a colon, then the tracker state name the user chose, so the file holds exactly two lines, keyed `work-started` and `pr-opened`. **Read/write rule:** on the **first** write-back prompt in a project, read the tracker's real workflow states, ask the user which state each of the two transition points maps to, and record the answers in this file. On every later prompt and every later ticket, read the file and **never ask again**. The plugin ships no copy of this file; it appears when the user first answers the mapping question.

**Silent skip.** When **no tracker tool is available**, every tracker step is skipped with **no prompt, no notice, and no error**, and the verb completes exactly as it would without the step. No mapping is read, written, or asked for. This matches the session-age gate's fail-silent posture.
...
6. **Verified foundation (INV-O1).** Never plan on top of, build atop, or report as done any work that has not passed independent verification. If circumstances force a re-plan, first re-verify or explicitly discard the unverified work it would otherwise build upon — pausing with unverified work sitting on disk is fine; quietly extending it is not. Rationale: unverified work compounds silently into false confidence the longer it goes unchecked.
7. **Status honesty (INV-O2).** Wherever work is referenced — journal entries, the index, a handoff brief to a subagent — its verification status travels with it. This generalises the journal's existing `trust: verified|asserted` field beyond the journal entry itself: any pointer to that work, anywhere, always carries the same verified-or-asserted status: a reference is never quietly stripped of it. Rationale: a downstream reader must always be able to tell re-derived fact from unverified claim.
...
- **Structure follows write-targets (INV-O3).** Whether tasks are dispatched together in a parallel wave is always bounded by their write-targets being disjoint, never forced merely by how many acceptance criteria or requirements they carry. A large AC/requirement count is never on its own a reason to fan a dispatch out into more parallel waves; overlapping write-targets always force serialisation regardless of how few ACs are involved. Rationale: concurrent writers to the same target are the real hazard parallel dispatch must avoid — AC count is not a proxy for that hazard.
...
**A PR URL is recorded as an entry under the existing `refs` field** — the same way a task or verification path is recorded. It is a pointer to canonical content held by the git host, so it belongs in `refs`. **No new frontmatter field is added to the journal schema for it**, and no PR field is added anywhere else (see "One status store" above).
...
3. Keep only **live pointers** in `initiatives.md`: the ACTIVE row, any parked-but-live initiatives, and the project-level artefacts (`file-structure.md`, `namespaces`, `tracker-states`) — nothing else.
...
**Index-bloat guard — generalizing this policy to the per-initiative `index.md` (A1–A4).** This same policy generalizes to the active initiative's own **index** `.workspace/<slug>/memory/index.md` (no new file or system introduced). It shares the same trigger style as the registry check above — size-triggered, measure-then-decide, human-prompt-gated — but with a **deliberately different action**: because `index.md` is **overwritten**, not appended to (see "Index schema" above), there is no superseded detail to relocate, so this guard never archives anything. **Already location-agnostic — no firing-point edit needed:** this guard's firing points are named by whichever procedure invokes it (Bootstrap/Teardown/Switch/Create), not restated here, so it requires no wording change for Switch/Create wiring.

- **Trigger:** the **byte size of `index.md`** (a plain `wc -c` / byte-length measurable — no count, hybrid, or "verbose"/semantic heuristic) compared against a **single named, documented, tunable threshold: default ~10–15 KB (provisional)**. As with the registry's ~10–12 KB default, this is a documented starting point, not a data-derived pick — **measure-then-decide**: only lower or otherwise retune it once real index sizes in practice warrant it; never hard-pick a number from a guess. This index threshold is **deliberately independent** of the registry consolidation threshold above — distinct file, distinct value; retuning one never implies retuning the other.
- **On-trip action — prompt-gated re-derivation nudge, not archival:** if size ≥ threshold, surface the one-line prompt: `index is N KB ≥ threshold T — re-derive it lean (pointers + short summaries only)? y/n`, mirroring the registry-consolidation prompt style. A "y" answer re-derives the index **lean** — pointers and short summaries only, never the authoritative payload, per the existing "index = pointers only / reference, never copy" rules (see "Precedence" above) — regenerating it from the initiative's `journal.md` plus its canonical artefacts (per "Rebuild from sources" above), which is lossless because the index holds no unique authoritative data.
- **Never automatic:** exactly as the registry check — this only ever surfaces the one-liner and waits on the human's answer; nothing rewrites the index without a "y". This check is advisory only.
- **Because `index.md` is overwritten, not append-only, this guard does NOT move index detail to `archive/`** — that relocation mechanism applies to the append-only `journal.md`/`events.jsonl`/`initiatives.md` above, not here; there is nothing to relocate, only to regenerate.
- **Ownership and mechanism unchanged:** the orchestrator remains the sole actor; this adds **no new agent** (canonical 8-agent set unchanged), **no new env var**, and **no inference/similarity/embedding/semantic** signal — it reuses the existing consolidation-policy vocabulary/machinery exactly as the registry generalization above does.

**Epic/granularity guard — generalizing this policy to initiative growth itself (B1–B4).** This same policy generalizes one further hop to the **active initiative as a whole**, surfacing when a single initiative may have grown too large to keep sharing one journal/index — but with a **deliberately different action**: this guard never archives anything, and it never gates on a "y" at all; it is a pure advisory notice.

- **Trigger — a single deterministic signal:** the **journal entry count** of the active initiative's `.workspace/<slug>/memory/journal.md`, measured with `grep -c '^timestamp:'` — no inference, similarity, or semantic judgement, and no second or hybrid signal alongside it. This count is compared against a **single named, documented, tunable threshold: default ~30 entries (provisional)**. As with the registry and index-bloat thresholds above, this is a documented starting point, not a data-derived pick — **measure-then-decide**: only retune it once real per-initiative journal-entry counts in practice warrant it; never hard-pick a number from a guess.
- **On-crossing action — advisory prompt at Switch/Create/Teardown, no gate:** if the count ≥ threshold, surface the one-line notice: `this initiative has grown to N — it may be Epic-sized; consider splitting into tighter initiatives`. Unlike the registry and index-bloat checks above, this notice is **not** phrased as a `y/n` question and **requires no answer** — it never blocks, delays, or gates Switch, Create, or Teardown in any way; the invoking procedure proceeds exactly as it would without the notice.
- **Advisory only — never acts on its own:** this guard **never auto-splits** the initiative, **never creates or switches** initiatives, and **never blocks teardown**. It is strictly informational, left for the human to act on (or ignore) at their own discretion.
...
**Producer of the `git-push` and `pr-create` events.** Both events come from the orchestrator's own git actions (see "Git actions" above), performed at flow step 5 — **after a task passes verification**, where you commit, then ask before push, then ask before opening the PR. A `git-push` event is written when a push runs; a `pr-create` event is written when a PR is opened. No agent and no other step produces them, and a prompt answered "n" produces no event.
