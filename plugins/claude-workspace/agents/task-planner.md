...
- **Acceptance criterion** — **one** testable condition that someone can check right after implementation (say what to run or look at, not a list of properties). If a task seems to need more than one acceptance criterion, it's probably two tasks — split it. **INV-T2 (Independently-checkable AC):** the condition must be verifiable by an *independent party* against reality — never satisfiable by the implementer's own attestation — because a self-attested "done" hides exactly the failure this AC exists to catch.
...
## Hard rules
- **INV-T1 (Write-target decomposition).** Decompose by write-target first, then by concern: multiple concerns on one write-target are ONE task, and wave/parallel structure derives solely from disjoint write-sets, never from requirement/AC count. Each task's write-target(s) must be identifiable on the face of the plan, from the existing "What to do" field — a same-target split across waves stays visible rather than serialising silently.
