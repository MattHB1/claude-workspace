...
### Removed
- **`felt` self-rating** — the orchestrator no longer asks for a `waiting`/`rework`/`steering`
  rating when an initiative closes. The `felt` journal field is gone from `SKILL.md`,
  `docs/memory.md`, and `benchmark/reference-task.md`. Old journal entries that hold `felt` stay
  valid. The deterministic signals (verifier `outcome:`, Metric 4b, edit-churn, the Repeated-FAIL
  guard) are unchanged.
